 ...
 *
 * @return {Object} Object with `sigs` {Array} of {Object} (tag/value from signature) and `err` {String} if sanity check fails.
 */
...
function DKIMVerify(email) {
    ...
    for (i = 0; i < headerArray.length; i++) {
      if (sig = headerArray[i].match(/^dkim\-signature:\s*(.*?)$/i)) {
        err = "";
        sig = sig[1].replace(/\s/, '');
        break; // only look at the first dkim-signature header for now
      }
    }
    if (err) {
      return {err: err};
