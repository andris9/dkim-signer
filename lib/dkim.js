...
function verifyMessageSignature(headers, body, sig, sigHeader, rec) {
  ...
  var hcanon = 'simple',
      ...
      sigHashAlgo = rec.h ? rec.h : sig.a.split('-')[0];
  ...
  // verify body hash
  if (!(sigHashAlgo in hashAlgos)) {
    return {ok: false, msg: 'Invalid hashing algorithm'};
  }
  bodyHash = hashAlgos[sigHashAlgo](bodyCanon, 'base64');
  if (sig.bh != bodyHash) {
...
var DKIMCanonicalizer = {
    ...
    simpleHeaders: function(headers, fieldNames) {
        ...
        for (i = headerChunks.length - 1; i >= 0; --i) {
          var chunk = headerChunks[i],
              lastChunk = headerChunks[i - 1];
          if (chunk.length == 0) {
            continue;
          }
          if (chunk.match(/^\r?\n|\r$/) || chunk.match(/^ +/)) {
            headerChunks[i - 1] = lastChunk + chunk;
          } else {
            headerLines.unshift(chunk);
          }
        }

        // Form a header name -> line number index
        headerLines.forEach(function(hdr, idx) {
        ...
        includedHeaderNames.forEach(function(hdrName) {
          if (hdrName in headerIndex) {
            ...
            foundHeaders += headerLines[lines.pop()];
