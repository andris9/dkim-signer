 ...
 *
 * @memberOf dkimsign
 * @param {String} list Semicolon-separated k=v list
...
function tagObject(listStr, tagSpec) {
  ...
  listStr.split(/\s*;\s*/).map(function(kv) {
    ...
    if (tmp) {
      if ((tagSpec.required[tmp[1]] || tagSpec.optional[tmp[1]]) && tags.hasOwnProperty(tmp[1])) {
 ...
 *
 * @return {Object} The `err` key will bet set if hash verification fails.
 */
function invalidHashes(headers, body, sig, rec) {
  ...
  if (hcanon == 'relaxed') {
    ...
    console.log('headers: '+ JSON.stringify(headCanon));
  } else {
    throw new Error('Simple header canonicalization is not implemented');
    //headCanon = DKIMCanonicalizer.simpleHeaders();
  }

  // TODO: create a signer here and see why things are different
  console.log('sig.b='+ sig.b);

  var pemKey =
    '-----BEGIN PUBLIC KEY-----\n'+
    rec.p.replace(/(.{1,64})/g, '$1\n')+
    '-----END PUBLIC KEY-----\n';
  if (!verifier.verify(pemKey, sig.b, 'base64')) {
    return {err: "Signature could not be verified"};
  }
  return undefined;
}
...
function DKIMVerify(email) {
    ...
    return module.exports.KeyFromDNS(sig.s, sig.d)
      ...
      .then(function(rec) {
        err = invalidHashes(headers, body, sig, rec);
        if (err) {
          sig.result = false;
          sig.issue_desc = err;
        } else {
      ...
      .fail(function(error) {
        console.log('chained failure handler for '+ sig.s +': '+ error);
      });
}
