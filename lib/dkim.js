...
module.exports.requiredTags = requiredTags;

var requiredTags = ['v', 'a', 'b', 'bh', 'd', 's', 'h'];
...
/**
 * <p>Perform sanity checks on a DKIM signature.</p>
 *
 * @memberOf dkimsign
 * @param {String} sig DKIM-Signature header value
 *
 * @return {Object} Object with `sigs` {Array} corresponding to DKIM-Signature header and `err` {String} if sanity check fails.
 */
function sigSanity(sig) {
  ...
  for (var tagIdx in requiredTags) {
    var tagName = requiredTags[tagIdx];
...
function DKIMVerify(email) {
    ...
    for (i = 0; i < headerArray.length; i++) {
      if (sig = headerArray[i].match(/^dkim\-signature:\s*(.*?)$/i)) {
        err = "";
        sig = sig[1].replace(/\s/, '');
        break; // only look at the first dkim-signature header
      }
    }
    if (err) {
      return {err: err};
    ...
    var tags = sigSanity(sig);
    if (tags.err) {
      return {err: tags.err};
    }

    // now we're talking to network services, so we switch to promise mode
    return module.exports.KeyFromDNS(tags.s, tags.d).then(function(dnsval) {
      console.log('got ('+ dnsval +') from KeyFromDNS');
      rv.sigs.push(tags);
      // TODO: remove headers from top, down to first dkim-signature and re-run validation
      return rv;
    });
}
